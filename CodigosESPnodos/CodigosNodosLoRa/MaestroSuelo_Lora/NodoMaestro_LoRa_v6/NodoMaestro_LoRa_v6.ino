/*
 * AgroSense - ESP32 WROVER-B DevKit  |  MAESTRO LoRa  — v6 (Nodo Suelo)
 * ========================================================================
 * Agrega soporte para Nodo 03 (sensor de suelo RS485):
 *   Trama: "$N:03,T:20.10,H:35.20,EC:312,PH:6.80,N:28,P:15,K:201\n"
 *
 * Nodos activos:
 *   Nodo 01: "$N:01,T:28.50,H:63.20\n"          → Temperatura + Humedad AIRE (SHT31)
 *   Nodo 02: "$N:02,M:1234.56\n"                 → Metano (MQ4)
 *   Nodo 03: "$N:03,T:20.10,H:35.20,EC:312,..."  → Suelo RS485 (SN-3002)
 *
 ...
 *
 * Monitor Serial (115200 baud) muestra todas las tramas recibidas en tiempo real.
 * Ejemplo de salida:
 *   [N01] T:28.50 H:63.20  RSSI:-62 dBm
 *   [N02] CH4:1234.56 ppm  RSSI:-65 dBm
 *   [N03] T:20.10 H:35.20 EC:312 pH:6.80 N:28 P:15 K:201  RSSI:-58 dBm
 */
...
// ── Helper: extraer substring entre dos marcadores ────────────────────────────
// Busca "tag:" en frame y devuelve el valor hasta la siguiente coma o fin
...
void parseFrame(const String& frame) {
  ...
  // ── Nodo 01: Temperatura + Humedad aire (SHT31) ───────────────────────────
  if (nodeId == "01") {
    String tStr = extractField(frame, "T:");
    String hStr = extractField(frame, "H:");
    if (tStr.length() == 0 || hStr.length() == 0) return;
  ...
  // ── Nodo 02: Metano ────────────────────────────────────────────────────────
    ...
    String mStr = extractField(frame, "M:");
...
class WifiConfigCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* c) override {
    ...
    if (doc["password"].is<const char*>()) wifiPassword = doc["password"].as<String>();
    if (doc["api_key"].is<const char*>())  apiKey       = doc["api_key"].as<String>();
    saveConfig();
    Serial.printf("[WiFi] Config recibida via BLE: ssid=%s\n", wifiSsid.c_str());
    bool ok = connectWiFi();
...
void saveReadingToSPIFFS() {
  if (!gotValidHumidity && !gotValidSoil) return;
  ...
  if (lines >= MAX_RECORDS) {
    // Rotar: reescribir sin la primera línea
...
void sendHistoryBLE() {
  ...
  while (f.available()) {
    ...
    // Reenviar como JSON compacto
    // Campos: ts,t,a,co2,ch4,st,sh,ec,ph,n,p,k
    char json[220];
    int c1 = line.indexOf(',');
    if (c1 > 0) {
...
// ── Válidos con fallback por timeout ──────────────────────────────────────────
bool getValidReadings(float& outHum, float& outMethane) {
  ...
  outHum     = (node1Stale && gotValidHumidity) ? lastValidHumidity : currentAirHumidity;
  outMethane = (node2Stale && gotValidMethane)  ? lastValidMethane  : currentMethane;
...
void sendToAWS() {
  ...
  if (!ready) {
    Serial.println("[WiFi] Esperando datos de nodos — POST omitido");
    return;
  }
  ...
  snprintf(body, sizeof(body),
    ...
    currentTemperature, hToSend, currentCO2, mToSend,
    soilTemp, soilHum, soilEC, soilPH, soilN, soilP, soilK);
  int code = http.POST(body);
  Serial.printf("[WiFi] POST %d  T:%.1f H:%.1f CH4:%.2f  SoilT:%.1f pH:%.2f EC:%d\n",
    code, currentTemperature, hToSend, mToSend, soilTemp, soilPH, soilEC);
  http.end();
}
...
void setup() {
  ...
  Serial.println("=== AgroSense WROVER-B LoRa v6 — Nodo Suelo activo ===");
}
...
void loop() {
  ...
  // Avisos de silencio por nodo
  if (gotValidHumidity && (now - lastNode1Rx) > 300000)
    Serial.printf("[LoRa] AVISO: N01 silencioso hace %lu ms\n", now - lastNode1Rx);
  if (gotValidMethane && (now - lastNode2Rx) > 300000)
    Serial.printf("[LoRa] AVISO: N02 silencioso hace %lu ms\n", now - lastNode2Rx);
  if (gotValidSoil && (now - lastNode3Rx) > 300000)
