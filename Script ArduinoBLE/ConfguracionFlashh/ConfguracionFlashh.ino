 ...
 * - BLE: configuración WiFi + datos en vivo + control bomba + histórico
 * - SPIFFS: credenciales WiFi, API key e historial (persisten reinicios)
 * - WiFi: envío a AWS cada 30s (se configura desde la app)
 *
...
#include <LittleFS.h>
#define SPIFFS LittleFS   // alias para no cambiar el resto del código
#include <DHT.h>
...
Preferences prefs;  // Para guardar config WiFi/API en NVS (más robusto que SPIFFS para config)
...
String wifiSsid     = "";
String wifiPassword = "";
String apiKey       = "4a595fb1f56073960bd9f3c4eb6284d5974af900f3d50225";
...
bool loadConfig() {
  ...
  Serial.println("[TIME] Sincronizando hora NTP...");
  prefs.end();
...
bool timeReady() {
  ...
  if (!getLocalTime(&timeinfo)) {
    Serial.println("[TIME] No sincronizado aún");
    return false;
  }
  Serial.println("[TIME] OK sincronizado");
  return true;
}
...
// ── Callback: configuración WiFi desde app ─────────────────────────────────
// La app envía: "ssid|password|apikey"
...
// La app envía: "1" (encender) o "0" (apagar)
class PumpCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* pChar) override {
    ...
    // Notificar estado actual de vuelta
    char status[2] = { cmd, '\0' };
...
void readSensors() {
  ...
  else Serial.println("[DHT11] Error leyendo temperatura - usando último valor");

  if (!isnan(h)) currentAirHumidity = h;
  else Serial.println("[DHT11] Error leyendo humedad - usando último valor");

  // Humedad suelo simulada con variación suave
...
void saveReadingToSPIFFS() {
  ...
  // Rotación: eliminar primeras 50 líneas si llegamos al límite
  if (count >= MAX_RECORDS) {
    ...
    Serial.println("[SPIFFS] Rotación de historial");
  }
...
Serial.printf("[TIME] %s\n", timeStr); // tiempo en segundos desde 1970
...
snprintf(newLine, sizeof(newLine), "%ld,%.1f,%.1f,%.1f\n",
  ...
  if (f) {
    ...
    Serial.println("[SPIFFS] Error escribiendo historial");
  }
}

// ── Enviar histórico por BLE ──────────────────────────────────────────────
void sendHistoryBLE() {
  if (!SPIFFS.exists(HISTORY_FILE)) {
  ...
  while (f.available()) {
    ...
    // Parsear: millis,temp,airHum,soilHum
    int c1 = line.indexOf(',');
...
void sendToAWS() {
  if (apiKey.length() == 0) {
    Serial.println("[WiFi] Sin API key configurada - omitiendo envío");
    return;
  }
  if (WiFi.status() != WL_CONNECTED) {
  ...
  if (httpCode == 200 || httpCode == 201) {
    ...
    Serial.printf("[WiFi] Error HTTP: %d - %s\n", httpCode, http.getString().c_str());
  }
  http.end();
}
...
void setup() {
  ...
  // Aumentamos el número de handles para acomodar todas las características
  ...
  // 2. Lecturas en vivo (NOTIFY)
  pReadingsChar = pService->createCharacteristic(
    READINGS_UUID, BLECharacteristic::PROPERTY_NOTIFY
  );
  pReadingsChar->addDescriptor(new BLE2902());

  // 3. Control bomba (WRITE + READ + NOTIFY)
  ...
  // 4. Petición histórico (WRITE)
  pHistoryReqChar = pService->createCharacteristic(
    HISTORY_REQ_UUID, BLECharacteristic::PROPERTY_WRITE
  );
  pHistoryReqChar->setCallbacks(new HistoryRequestCallbacks());

  // 5. Datos histórico (NOTIFY)
  pHistoryDataChar = pService->createCharacteristic(
    HISTORY_DATA_UUID, BLECharacteristic::PROPERTY_NOTIFY
  );
  pHistoryDataChar->addDescriptor(new BLE2902());

  // 6. Configuración WiFi (WRITE)
  ...
  // Estado inicial
  String initStatus = WiFi.status() == WL_CONNECTED ?
    "CONNECTED" : (hasConfig ? "DISCONNECTED" : "NOT_CONFIGURED");
...
void loop() {
  ...
  // Leer sensores + enviar por BLE cada 2s
...
const int chunkSize = 20;  // BLE seguro
...
for (int i = 0; i < len; i += chunkSize) {
  ...
  delay(30);  // MUY IMPORTANTE (evita pérdida)
}

Serial.printf("[BLE CHUNKS] -> %s\n", payload.c_str());
    }
  }

  // Guardar en SPIFFS cada 60s
  if (now - lastSaveTime >= SAVE_INTERVAL) {
    lastSaveTime = now;
    saveReadingToSPIFFS();
  }

  // Enviar a AWS cada 30s
