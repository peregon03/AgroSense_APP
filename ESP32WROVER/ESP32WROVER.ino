/*
 * AgroSense - ESP32 WROVER-B DevKit
 * ==========================================
 * - SHT31 en GPIO21/GPIO22 (I2C): temperatura + humedad aire reales
 * - CO2:    simulado (reemplazar con sensor MQ-135 u otro)
 * - Metano: simulado (reemplazar con sensor MQ-4 u otro)
 * - LED en GPIO3: controlable desde app (simula motobomba)
 ...
 *
 * CAMBIOS RESPECTO AL NANO ESP32:
 *   - Wire.begin(21, 22) en lugar de Wire.begin(A4, A5)
 *   - API BLE v3.x usa String en lugar de std::string
 */
#include "time.h"
#include <Wire.h>
...
// ── CORRECCIÓN: deviceId como String (antes std::string) ──────────────────
String deviceId = "";
...
void notifyWifiStatus(const char* status) {
  if (pWifiStatusChar != nullptr) {
    pWifiStatusChar->setValue(String(status));
...
class WifiConfigCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* pChar) override {
    // ── CORRECCIÓN: String en lugar de std::string ──
    ...
    // val ya es String, no necesita conversión
...
class HistoryRequestCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* pChar) override {
    ...
    if (val == "GET") {
      ...
      pHistoryDataChar->setValue(String("CLEARED"));
...
// ── CORRECCIÓN: retorna String en lugar de std::string ────────────────────
...
void sendHistoryBLE() {
  if (!SPIFFS.exists(HISTORY_FILE)) {
    pHistoryDataChar->setValue(String("EMPTY"));
    pHistoryDataChar->notify();
    return;
  }
  ...
  File f = SPIFFS.open(HISTORY_FILE, "r");
  if (!f) {
    pHistoryDataChar->setValue(String("ERROR"));
    pHistoryDataChar->notify();
    return;
  }

  pHistoryDataChar->setValue(String("START"));
  pHistoryDataChar->notify();
  delay(100);
  ...
  while (f.available()) {
    ...
    if (c1 > 0 && c2 > 0 && c3 > 0 && c4 > 0) {
      ...
      pHistoryDataChar->setValue(String(json));
      pHistoryDataChar->notify();
      delay(50);
      sent++;
    }
  ...
  pHistoryDataChar->setValue(String(endMsg));
  pHistoryDataChar->notify();
...
void setup() {
  ...
  // ── CORRECCIÓN: I2C con pines GPIO21/GPIO22 para el WROVER-B ─────────────
  ...
  // ── CORRECCIÓN: toString() devuelve String en v3.x, se asigna directo ────
  ...
  // ── CORRECCIÓN: setValue acepta String directamente ──────────────────────
  ...
  pWifiStatusChar->setValue(initStatus);
...
void loop() {
  ...
  if (now - lastBleTime >= BLE_INTERVAL) {
    ...
    if (bleConnected) {
      // ── CORRECCIÓN: payload como String, substring() en lugar de substr() ──
      String payload = buildReadingJson();
      const int chunkSize = 20;
      int len = payload.length();
      for (int i = 0; i < len; i += chunkSize) {
        String chunk = payload.substring(i, i + chunkSize);
  ...
  // Consultar programacion/override de bomba
